...
// const metaFbDescription = '<meta property="og:description" content="Social platform for poker players."/>'
...
//   res.sendFile(path.join(STATIC_FILES_DIRECTORY, 'index.html'))
  ...
  fs.readFile(INDEX_HTML_PATH, 'utf8', function (err, file) {
    ...
    const document = file.replace(metaFbUrl, `<meta property="og:url" content="${postUrl}"/>`)
      ...
      // .replace(metaFbDescription, '<meta property="og:description" content="Social platform for poker players."/>')
    res.send(document)
  })
})
